...
export default function AiSimulationPage() {
  ...
  useEffect(() => {
    operationsService.getAiPrediction().then((response) => setPrediction(response.data.data)).catch(() => null);
  }, []);

  const handlePanicCheck = async () => {
    const response = await operationsService.detectPanic({ text: panicText }).catch(() => null);
